...
if not TMDB_API_KEY:
    raise RuntimeError("TMDB_API_KEY missing.Put it in the .env as TMDB_API_KEY=xxxx")

app = FastAPI(title= "Movie Recommendation System", version="1.0")
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            details = f"TMDB request error: {type(e).__name__} |  {repr(e)}",
        )
...
async def tmdb_get_trailer_url(movie_id: int) -> Optional[str]:
    """
    Fetch movie videos from TMDB and extract the YouTube trailer URL.
    Accepts both "Trailer" and "Teaser" types.
...
async def tmdb_get_watch_providers(movie_id: int) -> Optional[dict]:
    """
...
async def tmdb_movie_details(movie_id: int) -> TMDBMovieDetails:
    ...
    trailer_url = await tmdb_get_trailer_url(movie_id)
    watch_providers = await tmdb_get_watch_providers(movie_id)
...
def load_pickles():
    ...
    try:
        ...
        raise RuntimeError(f"Required pickle file not found: {e}")
    except Exception as e:
        raise RuntimeError(f"Error loading pickles: {e}")
